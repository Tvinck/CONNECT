...
export default function ChatsPage() {
  return (
    <div className="flex h-[calc(100vh-48px)] -mx-4 sm:-mx-6 lg:-mx-8 -my-6 lg:-my-7">
      {/* Channels list */}
          ...
          {CHANNELS.map((c) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 h-9 rounded-lg text-[13px] font-medium tracking-tight mb-0.5 transition-all
                ${c.active ? 'bg-accent/15 text-accent border border-accent/30' : 'text-mute hover:text-white hover:bg-white/[0.04]'}`}
            >
              <Hash size={14} />
              <span className="flex-1 text-left">{c.label}</span>
              {c.unread ? (
                <span className="min-w-[18px] h-4 px-1 rounded bg-accent text-white text-[10px] font-bold inline-flex items-center justify-center">
                  {c.unread}
                </span>
              ) : null}
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-5 py-3.5 border-b border-line flex items-center gap-3">
          <Hash size={16} className="text-mute" />
          <span className="text-[14px] font-semibold tracking-tight">общий</span>
          <span className="text-[12px] text-mute ml-2">Всё подряд</span>
        </div>
        ...
        <div className="px-5 py-4 border-t border-line">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.03] border border-line hover:border-line2 transition-all">
            ...
            />
            <button className="w-8 h-8 rounded-lg bg-accent/15 text-accent hover:bg-accent hover:text-white transition-all inline-flex items-center justify-center">
              <Send size={15} />
            </button>
          </div>
        </div>
      ...
      <div className="w-[200px] shrink-0 border-l border-line hidden xl:flex flex-col">
        <div className="px-4 py-4 border-b border-line">
          <span className="text-[12px] text-mute uppercase tracking-[0.12em] font-semibold">Участники · {MEMBERS.length}</span>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          {MEMBERS.map((m, i) => (
            <button key={i} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-white/[0.04] transition-all">
