export default function handler(req, res) {
  ...
  // Use the host from the request headers, fallback to localhost
  const host = req.headers.host || 'localhost:5174';
  const protocol = host.includes('localhost') ? 'http' : 'https';
  
  // The URL where the iPhone will POST the device info (points back to bazzar-serts)
...
<dict>
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="bazzar-market-enroll.mobileconfig"');
