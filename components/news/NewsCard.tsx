...
export function NewsCard({ news, isRead = true }: { news: any, isRead?: boolean }) {
  ...
  return (
    <Link href={`/news/${news.id}`} className={`block border bg-card rounded-2xl p-5 hover:border-accent/30 hover:shadow-lg transition-all duration-200 relative ${isRead ? 'border-line' : 'border-accent/50 shadow-md shadow-accent/5'}`}>
      {!isRead && (
        <span className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 bg-accent rounded-full border-2 border-bg shadow-sm" />
      )}
        ...
        {news.tags && news.tags.length > 0 && (
          <div className="flex gap-2">
            {news.tags.map((tag: string) => (
              <span key={tag} className="px-2.5 py-1 rounded-full bg-accent/10 text-accent text-[10px] font-bold tracking-wide uppercase">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="mt-4 pt-4 border-t border-line text-[12px] text-accent font-semibold flex justify-end">
